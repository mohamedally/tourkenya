...
def description():
    if request.method == "POST":
        ...
        resorts = db.execute("SELECT * FROM resorts JOIN wildlife ON resorts.place = wildlife.name WHERE wildlife.name=:name",
                            name = key)
        resort_list = []
        ...
        return render_template("test.html", summary = summary, images=page.images,name = page.title, resorts = resort_list, attraction_list=attraction_list, company_list=company_list)
...
def cart():
    if request.method == "POST":
        if request.form.get("hotel_days"):
            hotel_price = request.form.get("hotel_price")
            hotel_days = request.form.get("hotel_days")
            hotel_total = hotel_price * hotel_days
            total = hotel_total

            db.execute("INSERT INTO cart (user_id, hotel_days, hotel_total, total) VALUES (:user_id, :hotel_days, :hotel_total, :total",
                   user_id = session["user_id"], hotel_days=hotel_days, hotel_total=hotel_total, total=total)

        if request.form.get("safari_days"):
            safari_days = request.form.get("safari_days")
            company = request.form.get("company")

            company_details = db.execute("SELECT * FROM companies WHERE name = :company",
                                     company = company)
            safari_total = int(company_details[0]["rate"]) * safari_days

            current_total = db.execute("SELECT total FROM cart WHERE user_id = :user_id",
                                       user_id=session["user_id"])
            total = safari_total + current_total[0]["total"]

            db.execute("UPDATE cart SET company = :company, safari_total = :safari_total, total = :total",
                       company=company, safari_total=safari_total, total=total)
                       
            return render_template("cart.html",safari_days=safari_days, company=company, safari_total=safari_total, current_total=current_total, total=total, hotel_days=hotel_days )
